								...
								<table border=0 cellspacing=0 cellpadding=0 width=100% class="small">
								   ...
								   {if $divName eq 'basicTab'}
									...
									{if $AVAILABLE_PRODUCTS eq true}
										{include file="modules/Receiptcards/ProductDetailsEditView.tpl"}
									{else}
...
var oldcontrolvalue = "{$PARENT_ID}";
{*
fldId = getObj("parent_id");
fldId.setAttribute ('onChange', 'alert("Hi")', 0);
 fldId.setAttribute ('onchange', 'alert("Hi")', 0); *}

function control_parent_id()
{ldelim}
...
{ldelim}
   ...
   parentfldType = getObj("parent_type");

   if (parentfldType.value == "PurchaseOrder&action=Popup")
   {ldelim}
        window.document.EditView.action.value = 'EditView';
     	window.document.EditView.submit();
   {rdelim}
   
{rdelim}

window.setInterval("control_parent_id()", 500);
</script>